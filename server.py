...
def check_company():
    ...
    try:
        egr_resp = requests.get('https://api-fns.ru/api/egr', params={'req': inn, 'key': FNS_API_KEY}, timeout=10)
        ...
        result = {'inn': inn, 'name': 'Неизвестно', 'status': 'Неизвестно', 'risk': 0, 'revenue': None, 'profit': None, 'ens_debt': None, 'blocked': False, 'suspended': False}
        ...
        if isinstance(check_data, list) and check_data:
            ...
            if ci:
                c = ci[0]
                if c.get('МассовыйАдрес'): result['risk'] += 20
                if c.get('МассовыйРуководитель'): result['risk'] += 15
                if c.get('НедостоверныеСведения'): result['risk'] += 30
                if c.get('ДисквалифицированныеЛица'): result['risk'] += 25
        ...
        if result['status'] == 'Ликвидировано': result['risk'] = 100
